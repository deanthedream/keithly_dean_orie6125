...
for i in np.arange(ArrayLength-1)+1:#The length of each array to sort
    unsorted = np.asarray(random.sample(range(1,(ArrayLength-1)*10),i))#np.unique(np.random.randint(0,high=1000, size=i))
    sortedArrayQS, timeQStmp = hS.quick_sort(unsorted.copy())
    sortedArrayIS, timeIStmp = hS.insertion_sort(unsorted.copy())
    #sortedArrayQS2, timeQS2tmp = hS.myQS2(unsorted.copy(),0,unsorted.copy().shape[0]-1)
